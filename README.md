...
只要有任何一個變數是無號數，Verilog 會將整個運算式強制轉換為無號數來計算。這會導致負數變成極大的正數。
解法： 使用 $signed() 系統函數來強制轉換，並確保等號兩邊的運算元性質一致。



如果希望保留兩個 N-bit 數相加的完整結果，先把兩個運算元都擴成 N+1 bit 再做加法
N bit + N bit ⇒ 最好使用 N+1 bit 加法器 減法一樣    沒有真正的減法器，減法是透過「加上 2 的補數」來實現的
兩個 N bit 的數字相加，結果需要 N+1 bit 才保證不溢位。
100 -4
100 -4
---
000 -8
...
1000 4-bit signed就是-8完全正確。


$N$ bit 乘以 $M$ bit，結果必須是 $N+M$ bit。如果位元數給錯，高位的資料會直接被截斷（Truncated）。
不要為了乘以常數（如乘以 3）就用乘法器，盡量用移位加法（a * 3 寫成 (a << 1) + a）來節省資源。
乘法乘完要位移>>>回去 或取部分 data[39:12]

很多時候可以把「除法」轉化為「乘上倒數」。例如要除以 5，可以改成先乘以 0.2 的定點數（乘法可以合成）。

固定小數點設計中，直接截斷通常會引入偏差，很多演算法會在右移前先加上半個 LSB 來實現四捨五入。
